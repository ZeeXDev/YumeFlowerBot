...
async def gestion_owner_menu(message: Message):
    ...
    try:
        ...
        text = (
            ...
            f"<i>Sélectionnez un bot à gérer:</i>"
        )
...
async def show_bot_selection(message: Message, bots: List[Dict]):
    ...
    text = (
        f"<b>⚙️ Sélectionnez un bot à gérer</b>\n\n"
...
async def show_gestion_menu(
    ...
    try:
        ...
        text = (
            ...
            f"🤖 <b>Nom:</b> @{bot_data['bot_username']}\n"
            f"🆔 <b>ID:</b> <code>{bot_id}</code>\n"
            f"{role_display}\n\n"
            f"<b>🔗 Identifiants:</b>\n"
            f"🆔 <b>ID_PUBS:</b> <code>{id_codes.get('id_pubs', 'N/A') if id_codes else 'N/A'}</code>\n"
            f"🔑 <b>ID_CODE:</b> <code>{id_codes.get('id_code', 'N/A') if id_codes else 'N/A'}</code>\n\n"
            f"<b>📊 Statistiques:</b>\n"
            ...
            f"📸 Photo démarrage: {has_photo}\n"
            f"💬 Message démarrage: {has_message}\n"
            f"🔘 Boutons perso: {btn_count}\n"
        ...
        # Ligne 1: Apparence
        buttons.append([
            InlineKeyboardButton("📸 Photo", callback_data=f"gestion_photo_{bot_id}"),
            InlineKeyboardButton("💬 Message", callback_data=f"gestion_msg_{bot_id}")
        ])
        
        # Ligne 2: Interactions
        buttons.append([
        ...
        # Ligne 4: Monétisation et stats (MAITRE et OWNER uniquement)
        if role in ['owner', 'maitre']:
            buttons.append([
                InlineKeyboardButton("💰 Gains", callback_data=f"gestion_earnings_{bot_id}"),
                InlineKeyboardButton("📊 Stats avancées", callback_data=f"gestion_stats_{bot_id}")
            ])
            
            buttons.append([
                InlineKeyboardButton("🔄 Régénérer ID_CODE", callback_data=f"gestion_regen_{bot_id}"),
                InlineKeyboardButton("⚙️ Paramètres", callback_data=f"gestion_settings_{bot_id}")
            ])
        
        # Ligne 5: Actions rapides
        if role in ['owner', 'maitre']:
            status_action = "🔴 Désactiver" if bot_data.get('is_active') else "🟢 Activer"
            buttons.append([
                InlineKeyboardButton(status_action, callback_data=f"gestion_toggle_{bot_id}"),
                InlineKeyboardButton("🗑️ Supprimer", callback_data=f"gestion_delete_{bot_id}")
            ])
        
        # Bouton retour pour OWNER
        if role == 'owner':
...
async def gestion_photo_del_callback(client: Bot, callback: CallbackQuery):
    ...
    try:
        ...
        buttons = [
            [
                InlineKeyboardButton("✅ Oui, supprimer", callback_data=f"gestion_photo_del_confirm_{bot_id}"),
...
async def gestion_delchannel_callback(client: Bot, callback: CallbackQuery):
    ...
    try:
        ...
        buttons = [
            [
                InlineKeyboardButton("✅ Oui, supprimer", callback_data=f"gestion_delchannel_confirm_{bot_id}"),
...
async def gestion_admin_add_callback(client: Bot, callback: CallbackQuery):
    ...
    try:
        ...
        text = (
            ...
            f"<b>⚠️ Attention:</b> L'admin aura accès à la gestion du bot!\n\n"
            f"Envoyez <code>/annuler</code> pour annuler."
        )
